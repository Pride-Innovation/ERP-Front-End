...
const EXCLUDED_KEYS_COMMON = ["action", "image", "assetStatus", "status", "availability", "purchaseCost", "priority"];
const EXCLUDED_KEYS_COST = ["action", "image", "assetStatus", "status", "availability", "priority"];
const EXCLUDED_KEYS_IMAGE = ["action", "assetStatus", "status", "availability", "purchaseCost", "priority"];
const EXCLUDED_KEYS_STATUS = ["image", "action", "availability", "purchaseCost", "priority"];
const EXCLUDED_KEYS_BOOLEAN = ["image", "action", "status", "assetStatus", "purchaseCost", "priority"];
const EXCLUDED_KEYS_BOOLEAN_AVAILABILITY = ["image", "action", "status", "assetStatus", "purchaseCost"];
                ...
                : ['string'].includes(typeof value) && !(EXCLUDED_KEYS_IMAGE.includes(key)) && key === "image" ? ({ label: key, isImage: true })
                    : ['string'].includes(typeof value) && !(EXCLUDED_KEYS_STATUS.includes(key)) && ["status", "assetStatus"].includes(key) ? ({ label: key, isStatus: true })
                        : (typeof value === "string") && !(EXCLUDED_KEYS_BOOLEAN.includes(key)) && ["availability"].includes(key) ? ({ label: key, isBoolen: true })
                            : (typeof value === "string") && !(EXCLUDED_KEYS_BOOLEAN_AVAILABILITY.includes(key)) && ["priority"].includes(key) ? ({ label: key, isPriority: true })
